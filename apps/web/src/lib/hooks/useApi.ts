...
export function useFavoriteListings(params?: { page?: number; limit?: number }) {
  const { data: user } = useCurrentUser();
  return useQuery<ListingsPage>({
    queryKey: ['favorite-listings', params?.page || 1, params?.limit || 12],
...
export function useToggleFavoriteListing() {
  ...
  return useMutation({
    mutationFn: ({ listingId, isFavorited }: { listingId: string; isFavorited: boolean }) =>
      isFavorited ? removeFavoriteListing(listingId) : addFavoriteListing(listingId),
    onMutate: async ({ listingId, isFavorited }) => {
      await queryClient.cancelQueries({ queryKey: ['favorite-listing-ids'] });
      const previous = queryClient.getQueryData<string[]>(['favorite-listing-ids']);
      queryClient.setQueryData<string[]>(['favorite-listing-ids'], (old = []) =>
        isFavorited ? old.filter((id) => id !== listingId) : [...old, listingId],
      );
      return { previous };
    },
    onError: (_err, _vars, context) => {
      if (context?.previous !== undefined) {
        queryClient.setQueryData(['favorite-listing-ids'], context.previous);
      }
    },
    onSettled: () => {
...
export function useFavoriteProviders(params?: { page?: number; limit?: number }) {
  const { data: user } = useCurrentUser();
  return useQuery<ProvidersPage>({
    queryKey: ['favorite-providers', params?.page || 1, params?.limit || 12],
...
export function useToggleFavoriteProvider() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ providerId, isFavorited }: { providerId: string; isFavorited: boolean }) =>
      isFavorited ? removeFavoriteProvider(providerId) : addFavoriteProvider(providerId),
    onMutate: async ({ providerId, isFavorited }) => {
      await queryClient.cancelQueries({ queryKey: ['favorite-provider-ids'] });
      const previous = queryClient.getQueryData<string[]>(['favorite-provider-ids']);
      queryClient.setQueryData<string[]>(['favorite-provider-ids'], (old = []) =>
        isFavorited ? old.filter((id) => id !== providerId) : [...old, providerId],
      );
      return { previous };
    },
    onError: (_err, _vars, context) => {
      if (context?.previous !== undefined) {
        queryClient.setQueryData(['favorite-provider-ids'], context.previous);
      }
    },
    onSettled: () => {
