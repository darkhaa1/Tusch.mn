...
import { DollarSign, FilterX, MapPin, Search, X } from "lucide-react";
...
import { useListingLocations } from "@web/lib/hooks/useListings";
...
export default function ListingsFiltersBar({ onCreateListing }: Props) {
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      setPriceMinInput(
        nextQuery.minPrice !== undefined ? String(nextQuery.minPrice) : "",
      );
      setPriceMaxInput(
        nextQuery.maxPrice !== undefined ? String(nextQuery.maxPrice) : "",
      );
    };
  ...
  const hasActiveFilters = Boolean(
    category || hasSearch || hasPriceFilter || location,
  );
  ...
  const applyPriceFilter = () => {
    const min = priceMinInput.trim()
      ? Math.max(0, Math.floor(Number(priceMinInput)))
      : undefined;
    const max = priceMaxInput.trim()
      ? Math.max(0, Math.floor(Number(priceMaxInput)))
      : undefined;
