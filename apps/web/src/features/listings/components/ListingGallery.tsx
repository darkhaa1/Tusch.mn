  ...
  return (
    <div className="overflow-hidden">
      ...
      <div className="flex gap-3 overflow-x-auto p-4">
        {[displayedMain, ...imageUrls.filter((_, idx) => idx !== selectedIndex)].slice(0, 4).map((url, idx) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative h-20 w-28 shrink-0 overflow-hidden rounded-lg border transition",
        ...
        {!imageUrls.length && <div className="h-20 w-28 shrink-0 overflow-hidden rounded-lg border bg-muted" />}
      </div>
    </div>
  );
}
